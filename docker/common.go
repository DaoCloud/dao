...
func init() {
	...
	cmd.BoolVar(&commonFlags.Debug, []string{"D", "-debug"}, false, "启用调试模式")
	cmd.StringVar(&commonFlags.LogLevel, []string{"l", "-log-level"}, "info", "设置日志级别")
	cmd.BoolVar(&commonFlags.TLS, []string{"-tls"}, false, "使用传输层安全协议(TLS); 默认是 --tlsverify")
	cmd.BoolVar(&commonFlags.TLSVerify, []string{"-tlsverify"}, dockerTLSVerify, "使用传输层安全协议(TLS), 并进行远程验证")

	// TODO use flag flag.String([]string{"i", "-identity"}, "", "Path to libtrust key file")

	var tlsOptions tlsconfig.Options
	commonFlags.TLSOptions = &tlsOptions
	cmd.StringVar(&tlsOptions.CAFile, []string{"-tlscacert"}, filepath.Join(dockerCertPath, defaultCaFile), "仅信任由该CA签署的证书")
	cmd.StringVar(&tlsOptions.CertFile, []string{"-tlscert"}, filepath.Join(dockerCertPath, defaultCertFile), "TLS证书文件路径")
	cmd.StringVar(&tlsOptions.KeyFile, []string{"-tlskey"}, filepath.Join(dockerCertPath, defaultKeyFile), "TLS密钥文件路径")

	cmd.Var(opts.NewNamedListOptsRef("hosts", &commonFlags.Hosts, opts.ValidateHost), []string{"H", "-host"}, "Docker 引擎socket(s)连接至")
}
