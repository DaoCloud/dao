...
func Parse(cmd *flag.FlagSet, args []string) (*container.Config, *container.HostConfig, *networktypes.NetworkingConfig, *flag.FlagSet, error) {
	var (
		...
		flPrivileged        = cmd.Bool([]string{"-privileged"}, false, "为运行容器授予格外的特权")
		flPidMode           = cmd.String([]string{"-pid"}, "", "使用 PID 命名空间")
		flUTSMode           = cmd.String([]string{"-uts"}, "", "使用 UTS 命名空间")
		flPublishAll        = cmd.Bool([]string{"P", "-publish-all"}, false, "发布所有暴露的端口为随机端口")
		flStdin             = cmd.Bool([]string{"i", "-interactive"}, false, "即使没有附加，也请保持标准输入为打开状态")
		flTty               = cmd.Bool([]string{"t", "-tty"}, false, "分配一个伪终端")
		flOomKillDisable    = cmd.Bool([]string{"-oom-kill-disable"}, false, "禁用 OOM Killer")
		flOomScoreAdj       = cmd.Int([]string{"-oom-score-adj"}, 0, "调整主机的 OOM Tune host's OOM 参数(-1000 至 1000)")
		flContainerIDFile   = cmd.String([]string{"-cidfile"}, "", "容器 ID 写入文件")
		flEntrypoint        = cmd.String([]string{"-entrypoint"}, "", "覆盖镜像默认的 ENTRYPOINT ")
		flHostname          = cmd.String([]string{"h", "-hostname"}, "", "容器主机名")
		...
		flUser              = cmd.String([]string{"u", "-user"}, "", "用户名或是用户ID (格式: <名称|用户id>[:<控制组|控制组id>])")
		flWorkingDir        = cmd.String([]string{"w", "-workdir"}, "", "容器内部的工作路径")
		flCPUShares         = cmd.Int64([]string{"#c", "-cpu-shares"}, 0, "CPU 计算资源的值 (相对值)")
		...
		flCpusetCpus        = cmd.String([]string{"-cpuset-cpus"}, "", "允许容器执行的CPU核指定(0-3, 0,1): 0-3代表运行在0,1,2,3这4个核上")
		flCpusetMems        = cmd.String([]string{"-cpuset-mems"}, "", "允许容器执行的CPU内存所在核指定(0-3, 0,1): 0-3代表运行在0,1,2,3这4个核上")
		flBlkioWeight       = cmd.Uint16([]string{"-blkio-weight"}, 0, "磁盘IO设置(相对值), 从10到1000")
		flSwappiness        = cmd.Int64([]string{"-memory-swappiness"}, -1, "调整容器内存 swap 分区的频率阀值 (0－100)")
		flNetMode           = cmd.String([]string{"-net"}, "default", "连接一个容器到一个网络")
		flMacAddress        = cmd.String([]string{"-mac-address"}, "", "容器的 MAC 地址 (例如: 92:d0:c6:0a:29:33)")
		flIPv4Address       = cmd.String([]string{"-ip"}, "", "容器 IPv4 地址 (例如: 172.30.100.104)")
		flIPv6Address       = cmd.String([]string{"-ip6"}, "", "容器 IPv6 地址 (例如: 2001:db8::33)")
		flIpcMode           = cmd.String([]string{"-ipc"}, "", "使用 IPC 命名空间")
		flRestartPolicy     = cmd.String([]string{"-restart"}, "no", "容器退出时使用重启策略")
		flReadonlyRootfs    = cmd.Bool([]string{"-read-only"}, false, "挂载容器的根目录文件系统为只读模式")
		flLoggingDriver     = cmd.String([]string{"-log-driver"}, "", "为容器记录驱动日志")
		flCgroupParent      = cmd.String([]string{"-cgroup-parent"}, "", "容器的可选cgroup父路径")
		flVolumeDriver      = cmd.String([]string{"-volume-driver"}, "", "可供容器选择的存储卷驱动")
		flStopSignal        = cmd.String([]string{"-stop-signal"}, signal.DefaultStopSignal, fmt.Sprintf("停止容器的信号, %v by default", signal.DefaultStopSignal))
		flIsolation         = cmd.String([]string{"-isolation"}, "", "容器隔离级别")
		flShmSize           = cmd.String([]string{"-shm-size"}, "", "/dev/shm的大小, 默认值为 64MB")
	)

	cmd.Var(&flAttach, []string{"a", "-attach"}, "连接至标准输入，标准输出或标准错误")
	cmd.Var(&flBlkioWeightDevice, []string{"-blkio-weight-device"}, "磁盘IO值(相对设备值)")
	cmd.Var(&flDeviceReadBps, []string{"-device-read-bps"}, "限制读取设备的速度(每秒的字节数)")
	cmd.Var(&flDeviceWriteBps, []string{"-device-write-bps"}, "限制写入设备的速度(每秒的字节数)")
	cmd.Var(&flDeviceReadIOps, []string{"-device-read-iops"}, "限制读取设备的速度(每秒的IO数)")
	cmd.Var(&flDeviceWriteIOps, []string{"-device-write-iops"}, "限制写入设备的速度(每秒的IO数)")
	cmd.Var(&flVolumes, []string{"v", "-volume"}, "绑定挂载的存储卷")
	cmd.Var(&flTmpfs, []string{"-tmpfs"}, "挂载一个临时的文件系统目录")
	cmd.Var(&flLinks, []string{"-link"}, "为另一个容器指定链接")
	cmd.Var(&flAliases, []string{"-net-alias"}, "指定容器在网络范围的别名")
	cmd.Var(&flDevices, []string{"-device"}, "为容器指定一个主机设备")
	cmd.Var(&flLabels, []string{"l", "-label"}, "在容器上配置元数据")
	cmd.Var(&flLabelsFile, []string{"-label-file"}, "读入一行分隔文件的标签")
	cmd.Var(&flEnv, []string{"e", "-env"}, "设置环境变量")
	cmd.Var(&flEnvFile, []string{"-env-file"}, "读入一个文件的环境变量")
	cmd.Var(&flPublish, []string{"p", "-publish"}, "为主机发布一个容器的端口")
	cmd.Var(&flExpose, []string{"-expose"}, "暴露一个或一套端口")
	cmd.Var(&flDNS, []string{"-dns"}, "设置定制的域名服务器")
	cmd.Var(&flDNSSearch, []string{"-dns-search"}, "设置定制的域名搜索域")
	cmd.Var(&flDNSOptions, []string{"-dns-opt"}, "设置DNS选项")
	cmd.Var(&flExtraHosts, []string{"-add-host"}, "指定一个自定义的主机到IP的映射(host:ip)")
	cmd.Var(&flVolumesFrom, []string{"-volumes-from"}, "从指定容器挂载存储卷")
	cmd.Var(&flCapAdd, []string{"-cap-add"}, "指定 Linux 功能")
	cmd.Var(&flCapDrop, []string{"-cap-drop"}, "删除 Linux 功能")
	cmd.Var(&flGroupAdd, []string{"-group-add"}, "引入额外的控制组(groups)")
	cmd.Var(&flSecurityOpt, []string{"-security-opt"}, "安全选项")
	cmd.Var(flUlimits, []string{"-ulimit"}, "Ulimit 选项")
	cmd.Var(&flLoggingOpts, []string{"-log-opt"}, "记录驱动选项")
