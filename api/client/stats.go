...
func (cli *DockerCli) CmdStats(args ...string) error {
	...
	all := cmd.Bool([]string{"a", "-all"}, false, "显示所有容器(默认仅显示运行状态的容器)")
	noStream := cmd.Bool([]string{"-no-stream"}, false, "禁用实时数据并下拉第一次返回的结果")
	...
	printHeader := func() {
		...
		io.WriteString(w, "容器\tCPU %\t内存使用量/限制\t内存 %\t网络 I/O\t磁盘 I/O\n")
	}
	for _, n := range names {
