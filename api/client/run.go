...
func (cid *cidFile) Write(id string) error {
	if _, err := cid.file.Write([]byte(id)); err != nil {
		return fmt.Errorf("容器 ID 写入文件失败: %s", err)
	}
	cid.written = true
	return nil
}
...
func runStartContainerErr(err error) error {
	trimmedErr := strings.Trim(err.Error(), "Docker 引擎的返回错误: ")
...
func (cli *DockerCli) CmdRun(args ...string) error {
	...
	var (
		flAutoRemove = cmd.Bool([]string{"-rm"}, false, "容器退出时自动删除容器")
		flDetach     = cmd.Bool([]string{"d", "-detach"}, false, "后台运行容器并打印容器ID")
	...
	if len(hostConfig.DNS) > 0 {
		...
		for _, dnsIP := range hostConfig.DNS {
			if dns.IsLocalhost(dnsIP) {
				fmt.Fprintf(cli.err, "警告: 本地的DNS设定(--dns=%s) 在容器内可能失效。\n", dnsIP)
				break
			}
		}
	}
