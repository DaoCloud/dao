...
func (cli *DockerCli) CmdAttach(args ...string) error {
	...
	noStdin := cmd.Bool([]string{"-no-stdin"}, false, "不附着标准输入")
	proxy := cmd.Bool([]string{"-sig-proxy"}, true, "代理所有接受到的信号至进程")
	detachKeys := cmd.String([]string{"-detach-keys"}, "", "覆盖从一个容器停止附着的输入键顺序")
	...
	if !c.State.Running {
		return fmt.Errorf("您不能附着到一个停止的容器，请先启动此容器。")
	}

	if c.State.Paused {
		return fmt.Errorf("您不能附着到一个暂停的容器中，请先启动此容器")
	}
