...
func (cli *DockerCli) CmdLogin(args ...string) error {
	cmd := Cli.Subcmd("login", []string{"[SERVER]"}, Cli.DockerCommands["login"].Description+".\n如果没有制定服务器，Docker引擎会采用默认地址", true)
	cmd.Require(flag.Max, 1)

	flUser := cmd.String([]string{"u", "-username"}, "", "用户名")
	flPassword := cmd.String([]string{"p", "-password"}, "", "密码")
	flEmail := cmd.String([]string{"e", "-email"}, "", "邮箱")
	...
	if err != nil {
		if client.IsErrUnauthorized(err) {
			...
			if err2 := cli.configFile.Save(); err2 != nil {
				fmt.Fprintf(cli.out, "警告: 不能保存认证信息: %v\n", err2)
			}
		}
		return err
	}
	...
	fmt.Fprintf(cli.out, "警告: 登录凭证保存在%s\n", cli.configFile.Filename())
...
func (cli *DockerCli) configureAuth(flUser, flPassword, flEmail, serverAddress string) (types.AuthConfig, error) {
	...
	if flPassword == "" {
		...
		if flPassword == "" {
			return authconfig, fmt.Errorf("错误 : 需要密码")
		}
	}
